...
public class UserInfoValidation implements Serializable {
	...
	public void validateUserName(FacesContext context, UIComponent toValidate, Object value) throws ValidatorException {
		if (value != null && !value.equals("")) {
			...
			if (testUser != null) {
				FacesMessage msg = MessageFactory.getMessage("error.user.usernamealreadyexists");
	...
	public void validatePasswordConfirm(FacesContext context, UIComponent toValidate, Object value) throws ValidatorException {
		if (value != null && !value.equals("")) {
			...
			if (!passwordConfirm.equals(password)) {
				FacesMessage msg = MessageFactory.getMessage("error.user.passwordconfirm");
				throw new ValidatorException(msg);
			}
		}
	}
		...
		if (value != null && !value.equals("")) {
			...
			for (int i=0; i < field.length(); i++) {
				if (Character.isDigit(field.charAt(i))) {
					FacesMessage msg = MessageFactory.getMessage("error.user.cannotcontainsdigit");
					throw new ValidatorException(msg);
				}
			}
		}
		...
		if (value != null && !value.equals("")) {
	        ...
	        if (email != null) {
				if (!email.equals(emailConfirm)) {
					FacesMessage msg = MessageFactory.getMessage("error.user.emailconfirm");
					throw new ValidatorException(msg);
				}
				/***** check if email is not already used in db *********/
				User testUser = userService.getUserByEmail(email);
				if (testUser != null) {
					FacesMessage msg = MessageFactory.getMessage("error.user.emailalreadyexists");
					throw new ValidatorException(msg);
				}
	        }
		}
		...
		if (value != null && !value.equals("")) {
	        ...
	        if (signUserName != null) {
				...
				if (signUser == null || !signUser.getValidated()) {
					FacesMessage msg = MessageFactory.getMessage("error.user.invaliduserorpassword");
					throw new ValidatorException(msg);
