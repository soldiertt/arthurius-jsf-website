...
public class UserInfoRequestBean implements Serializable {
	...
	public String getConfirmEmailMessage() {
		...
		if (email != null && chuck != null) {
			...
			if (!user.getValidated() && user.getChuckCode().equals(chuck)) {
				...
				FacesMessage message = MessageFactory.getMessage("page.userreg_confirmemail.message");
				confirmEmailMessage = message.getSummary();
				confirmEmailOK = true;
			}
		}
		if (!confirmEmailOK) {
			FacesMessage message = MessageFactory.getMessage("page.userreg_confirmemail.errormessage");
			confirmEmailMessage = message.getSummary();
		}
	...
	public String actionUpdateUser() {
		...
		FacesMessage message = MessageFactory.getMessage("page.user_myprofile.profilesaved");
		msgInfo = message.getSummary();
		return "";
	}
