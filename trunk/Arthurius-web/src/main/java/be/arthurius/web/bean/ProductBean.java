...
public class ProductBean implements Serializable
{
   ...
   private Predicate[] getSearchPredicates(Root<Product> root)
   {
      ...
      String ref = this.searchProduct.getRef();
      if (ref != null && !"".equals(ref))
      {
         predicatesList.add(builder.like(root.<String> get("ref"), '%' + ref + '%'));
      }
      String type = this.searchProduct.getType();
      if (type != null && !"".equals(type))
      {
         predicatesList.add(builder.like(root.<String> get("type"), '%' + type + '%'));
      }
      String mark = this.searchProduct.getMark();
      if (mark != null && !"".equals(mark))
      {
         predicatesList.add(builder.like(root.<String> get("mark"), '%' + mark + '%'));
      }
      String name = this.searchProduct.getName();
      if (name != null && !"".equals(name))
      {
         predicatesList.add(builder.like(root.<String> get("name"), '%' + name + '%'));
      }
      String desc = this.searchProduct.getDescription();
      if (desc != null && !"".equals(desc))
      {
         predicatesList.add(builder.like(root.<String> get("description"), '%' + desc + '%'));
      }
